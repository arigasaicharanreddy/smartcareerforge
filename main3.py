...
# =========================================================
# LLM (HuggingFace) — reads token from .env
# =========================================================
...
def load_hf_client():
    ...
    if not token:
        ...
        st.stop()
    return InferenceClient(model=HF_MODEL_ID, token=token)

def _hf_chat(prompt: str, max_new_tokens: int = 512, temperature: float = 0.5) -> str:
...
@traceable(name="hf_llm_call", run_type="llm",
           metadata={"platform": "Smart CareerFordge", "model": "mistral-7b"})
def safe_llm_invoke(prompt: str, fallback: str = "Model timed out — please retry.",
                    feature_tag: str = "general") -> str:
    start_time = time.time()
    try:
        result  = _hf_chat(prompt)
        elapsed = round(time.time() - start_time, 3)
...
def _process_single_url(role_name, url, label=""):
    ...
    summary = safe_llm_invoke(prompt, feature_tag="url_summarizer")
...
- LLM (roadmap/ATS): mistralai/Mistral-7B-Instruct-v0.2 (HuggingFace free inference API)
...
- `HUGGINGFACEHUB_API_TOKEN` — HuggingFace token for Mistral-7B inference
