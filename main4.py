...
def _alert_adzuna_down() -> None:
    _send_alert(
        ...
        priority="high", tags="rotating_light,briefcase",
    )
...
<style>
    ...
    .stButton > button {
        ...
        text-overflow: ellipsis !important; padding: 5px 10px !important;
        min-height: 32px !important; height: auto !important; line-height: 1.3 !important;
        width: 100% !important; display: flex !important; align-items: center !important;
        justify-content: center !important; transition: all 0.18s ease !important;
    }
    .stButton > button *, .stButton > button p, .stButton > button span, .stButton > button div {
        ...
        font-size: 0.75rem !important; font-weight: 600 !important; line-height: 1.3 !important;
        padding: 0 !important; margin: 0 !important;
    }
    .stButton > button:hover, .stButton > button:hover *, .stButton > button:hover span, .stButton > button:hover p {
        background: #2563EB !important; color: #FFFFFF !important; border-color: #2563EB !important;
    }
    .stButton > button[kind="primary"] {
        background: #22D3EE !important; color: #000000 !important; border: none !important;
        font-weight: 700 !important; font-size: 0.8rem !important; padding: 7px 18px !important;
    }
    ...
    .stDownloadButton > button {
        background: #FFFFFF !important; color: #000000 !important;
        border: 1px solid #E5E7EB !important; border-radius: 10px !important; font-weight: 600 !important;
    }
    ...
    .hero-title {
        ...
        filter: drop-shadow(0 1px 1px rgba(255,255,255,0.25)) drop-shadow(0 4px 12px rgba(0,0,0,0.8));
    }
    ...
    .stTabs [data-baseweb="tab"] { border-radius: 8px !important; color: #CBD5E1 !important; font-family: 'Syne', sans-serif !important; font-size: 0.85rem !important; font-weight: 500 !important; background: transparent !important; padding: 10px 18px !important; }
    .stTabs [aria-selected="true"] { background: transparent !important; color: #22D3EE !important; font-weight: 700 !important; border-bottom: 2px solid #22D3EE !important; border-radius: 0px !important; }
    ...
    [role="option"], [role="option"] *, [role="option"] span, [role="option"] div, [role="option"] p { color: #000000 !important; background: #FFFFFF !important; font-weight: 500 !important; }
    ...
    [data-testid="stFileUploader"] button, [data-testid="stFileUploaderDropzone"] button { background: #FFFFFF !important; color: #000000 !important; border: 1px solid #d1d5db !important; border-radius: 8px !important; font-weight: 600 !important; }
    ...
    .splash-logo { display: block; margin: 60px auto 0 auto; width: 100%; max-width: 720px; border-radius: 18px; box-shadow: 0 0 80px rgba(220,220,220,0.12); animation: splashFade 1.2s cubic-bezier(0.16,1,0.3,1) both; }
        ...
        try:
            ...
            if _ls_enabled() and _ls_client:
                try:
                    ...
                    if run:
                        run.metadata.update({
                            "feature": feature_tag, "temperature": temperature,
                            "max_tokens": max_tokens, "latency_sec": elapsed,
                            "attempt": attempt,
...
def _adzuna_request(what: str, page: int, fetch_count: int, location: str,
                    max_days_old: float = None) -> list:
...
def _adzuna_count(what: str) -> int:
    try:
        app_id, app_key = _get_adzuna_credentials()
        params = {"app_id": app_id, "app_key": app_key, "results_per_page": 1, "what": what}
...
def fetch_adzuna_jobs(role, location="india", results=7, page=1, exp_level="",
                      exclude_urls=None, hours_old=None):
    exclude_urls = exclude_urls or []
    max_results  = min(results, 10)
    fetch_count  = max_results + len(exclude_urls) + 5

    api_max_days = None
    if hours_old is not None:
        api_max_days = max(1, round(hours_old / 24, 1))
    ...
    def _parse_jobs(raw_list):
        ...
        for job in raw_list:
            ...
            jobs.append({
                ...
                "salary_min":  sal_min,
                "salary_max":  sal_max,
                "description": (job.get("description", "") or "")[:400].strip() + "…",
                "url":         redirect,
    ...
    try:
        ...
        raw1 = _adzuna_request(q1, page, fetch_count, location, max_days_old=api_max_days)
        ...
        if core and core.lower() != q1.lower():
            raw2 = _adzuna_request(core, page, fetch_count, location, max_days_old=api_max_days)
        ...
        if fallback_kw.lower() not in (q1.lower(), core.lower()):
            raw3 = _adzuna_request(fallback_kw, 1, fetch_count, location, max_days_old=api_max_days)
...
def safe_llm_invoke(prompt: str,
    ...
    try:
        result = _call_groq_direct(
            ...
            max_tokens=1024, temperature=0.3, feature_tag=feature_tag,
        )
        elapsed = round(time.time() - start_time, 3)
        if elapsed >= ALERT_THRESHOLDS["latency_warning_sec"]:
            _alert_slow_response(feature_tag, elapsed)
        if _ls_enabled():
            try:
                run = get_current_run_tree()
                if run:
                    run.metadata.update({"feature": feature_tag, "latency_sec": elapsed})
            except Exception:
                pass
        return result
    except Exception as e:
...
# =========================================================
# DYNAMIC SKILL DICTIONARIES  ── FIXED: expanded aliases + implies
# =========================================================
...
def build_dynamic_skill_dicts(_role_data_map: dict):
    # ── FIXED: greatly expanded bidirectional alias dict ──────────────────────
    SKILL_ALIASES = {
        # JS / TS
        ...
        "cicd": "continuous integration", "ci cd": "continuous integration",
        # ML / AI
        ...
        "gen ai": "generative ai", "generative ai": "gen ai",
        "genai": "generative ai",
        # LLMs
        "llm": "large language models", "large language models": "llm",
        "llms": "large language models", "llm models": "large language models",
        # RAG
        "rag": "retrieval augmented generation",
        "retrieval augmented generation": "rag",
        "retrieval-augmented generation": "rag",
        # Fine-tuning variants
        "fine-tuning": "finetuning", "finetuning": "fine-tuning",
        "fine tuning": "fine-tuning", "finetune": "fine-tuning",
        # LoRA / QLoRA
        "lora": "low rank adaptation", "low rank adaptation": "lora",
        "qlora": "quantized lora", "quantized lora": "qlora",
        ...
        "lightgbm": "lgbm", "lgbm": "lightgbm",
        "catboost": "cat boost",
        # BERT / GPT
        "bert": "bidirectional encoder representations",
        "gpt": "generative pretrained transformer",
        # OpenCV
        "opencv": "open cv", "open cv": "opencv",
        # Pandas / NumPy shortcuts
        "pandas": "pd", "pd": "pandas",
        "numpy": "np", "np": "numpy",
        "matplotlib": "plt", "plt": "matplotlib",
        # FastAPI / Streamlit
        "fastapi": "fast api", "fast api": "fastapi",
        "streamlit": "stream lit", "stream lit": "streamlit",
        # MLOps / MLflow
        "mlops": "ml operations", "ml operations": "mlops",
        "mlflow": "ml flow", "ml flow": "mlflow",
        # LangChain ecosystem
        "langchain": "lang chain", "lang chain": "langchain",
        "langgraph": "lang graph", "lang graph": "langgraph",
        "langsmith": "lang smith", "lang smith": "langsmith",
        "crewai": "crew ai", "crew ai": "crewai",
        "llamaindex": "llama index", "llama index": "llamaindex",
        "llama-index": "llamaindex",
        # Vector DBs
        "chromadb": "chroma db", "chroma db": "chromadb",
        "chroma": "chromadb",
        "pinecone": "pine cone",
        ...
        # BI Tools
        "power bi": "powerbi", "powerbi": "power bi", "power-bi": "power bi",
        "tableau": "data visualization",
        # Workflow / streaming
        "airflow": "apache airflow", "apache airflow": "airflow",
        "kafka": "apache kafka", "apache kafka": "kafka",
        "spark": "apache spark", "apache spark": "spark",
        # SQL/NoSQL
        ...
        "dw": "data warehouse", "data warehouse": "dw", "dwh": "data warehouse",
        # Security
        "soc": "security operations center",
        "iam": "identity access management", "identity access management": "iam",
        "sast": "static application security testing",
        "dast": "dynamic application security testing",
        # Git / VCS
        "vcs": "version control", "scm": "source control management",
    ...
    # ── FIXED: expanded implies dict ─────────────────────────────────────────
    IMPLIES = {
        "pytorch":        ["deep learning", "neural networks", "machine learning", "python", "transformers"],
        "torch":          ["deep learning", "neural networks", "machine learning", "python"],
        ...
        "sklearn":        ["machine learning", "data preprocessing", "python"],
        "langchain":      ["llm", "prompt engineering", "rag", "api", "python"],
        "langgraph":      ["llm", "langchain", "python"],
        "huggingface":    ["llm", "transformers", "nlp", "python", "fine-tuning"],
        "transformers":   ["nlp", "llm", "python", "deep learning"],
        "openai":         ["llm", "api", "prompt engineering", "gpt"],
        ...
        "fastapi":        ["python", "backend", "rest api", "api"],
        "streamlit":      ["python", "data science", "visualization"],
        "spring":         ["java", "backend", "rest api"],
        "pandas":         ["data analysis", "python", "data wrangling"],
        "numpy":          ["data analysis", "python", "linear algebra"],
        "matplotlib":     ["data visualization", "python"],
        "seaborn":        ["data visualization", "python", "statistics"],
        "spark":          ["big data", "scala", "python", "distributed computing"],
        "kafka":          ["streaming", "event driven", "distributed systems"],
        "aws":            ["cloud", "devops"],
        "azure":          ["cloud", "devops"],
        "gcp":            ["cloud", "devops"],
        "terraform":      ["infrastructure as code", "devops", "cloud"],
        "ansible":        ["infrastructure as code", "devops", "linux"],
        ...
        "redis":          ["nosql", "caching", "database"],
        "elasticsearch":  ["nosql", "search", "database"],
        "git":            ["version control", "collaboration"],
        "jenkins":        ["ci/cd", "devops", "automation"],
        "github actions": ["ci/cd", "devops", "git"],
        "rag":            ["llm", "vector database", "python", "langchain"],
        "fine-tuning":    ["llm", "deep learning", "python", "transformers"],
        "finetuning":     ["llm", "deep learning", "python", "transformers"],
        "lora":           ["fine-tuning", "llm", "pytorch"],
        "qlora":          ["fine-tuning", "llm", "pytorch", "lora"],
        "mlops":          ["devops", "machine learning", "python", "docker"],
        "mlflow":         ["mlops", "machine learning", "python"],
        "chromadb":       ["vector database", "rag", "python"],
        "xgboost":        ["machine learning", "python", "gradient boosting"],
        "lightgbm":       ["machine learning", "python", "gradient boosting"],
        "opencv":         ["computer vision", "python", "image processing"],
        "airflow":        ["data engineering", "python", "pipeline"],
        "dbt":            ["data engineering", "sql", "analytics"],
...
# =========================================================
# SKILL MATCHING HELPERS  ── FIXED throughout
# =========================================================
def clean_skill(s):
    s = s.lower().strip().strip("*-•()")
    s = re.sub(r"[^a-z0-9\s/\.\+#]", " ", s)
...
def parse_resume_skill_list(raw_text):
    _COMPOUND = {
        "LangChain","LangGraph","LangSmith","TensorFlow","PyTorch","OpenCV",
        "NumPy","SageMaker","MongoDB","PostgreSQL","GitHub","GitLab",
        "HuggingFace","OpenAI","DeepMind","FastAPI","ReactJS","VueJS",
        "NextJS","NodeJS","GraphQL","DevOps","MLFlow","MLOps",
        "MultiModal","AutoML","CrewAI","LlamaIndex","ChromaDB","LangSmith",
        "OpenDevin","LightGBM","XGBoost","CatBoost","SpaCy","AllenNLP",
    }
    text = raw_text

    placeholders = {}
    for i, name in enumerate(sorted(_COMPOUND)):
        ph = f"CMPD{i}X"
        placeholders[ph] = name.lower()
        text = re.sub(re.escape(name), ph, text, flags=re.IGNORECASE)

    text = re.sub(r"([a-z]{2,})([A-Z][a-z]{2,})", r"\1\n\2", text)
    text = re.sub(r"(?m)^[A-Za-z0-9 ,&/()\.\-]{3,}\s*:\s*", "\n", text)

    for ph, name in placeholders.items():
        text = text.replace(ph.lower(), name)
        text = text.replace(ph, name)

    def _expand_slash(m):
        a, b, suffix = m.group(1).strip(), m.group(2).strip(), m.group(3).strip()
        suffix = suffix.rstrip(", ")
        return f"{a} {suffix}, {b} {suffix}" if suffix else f"{a}, {b}"
    text = re.sub(
        r"([A-Za-z]+)/([A-Za-z]+)[^\S\n]+([A-Za-z][A-Za-z ,]*?)(?=[,;\n]|\Z)",
        _expand_slash, text,
    )
    ...
    parts = re.split(r"[,\n;|•]", text)
    _STOP = {"and","or","the","a","an","with","for","of","in","at","by","on","to","as","be","e","g","etc","r","c"}
    skills, seen = [], set()
    for p in parts:
        p = clean_skill(p)
        if len(p) > 1 and p not in seen and p not in _STOP and not re.match(r"^\d+$", p):
            seen.add(p)
            skills.append(p)
    return skills


# ── FIXED: reverse alias lookup added ────────────────────────────────────────
def expand_resume_skills(skill_list):
    expanded = set(skill_list)
    queue, visited = list(skill_list), set()
    # Build reverse-alias map: value → {all keys that map to it}
    _rev = {}
    ...
    while queue:
        ...
        if alias and alias not in expanded:
            expanded.add(alias)
            queue.append(alias)
        # Reverse aliases — every key that maps TO s
        for rev in _rev.get(s, set()):
            if rev not in expanded:
                expanded.add(rev)
                queue.append(rev)
        # Implications
        for implied in IMPLIES.get(s, []):
            ...
            if imp not in expanded:
                ...
                if a2 and a2 not in expanded:
                    expanded.add(a2)
                    queue.append(a2)
                # Reverse for implied
                for rev2 in _rev.get(imp, set()):
                    if rev2 not in expanded:
                        expanded.add(rev2)
                        queue.append(rev2)
    return expanded


# ── FIXED: bidirectional expansion ───────────────────────────────────────────
def expand_jd_skills(jd_skill_list: list) -> dict:
    ...
    for jd_skill in jd_skill_list:
        s = clean_skill(jd_skill)
        equivalents = {s}
        # Forward alias
        alias = SKILL_ALIASES.get(s)
        if alias:
            equivalents.add(alias)
        # Reverse aliases — all keys that map TO s or TO alias
        for rev in _rev.get(s, set()):
            equivalents.add(rev)
        if alias:
            for rev in _rev.get(alias, set()):
                equivalents.add(rev)
        # All keys whose value == s
        for k, v in SKILL_ALIASES.items():
            if v == s:
                equivalents.add(k)
        # Implications + their reverses
        for imp in IMPLIES.get(s, []):
            ic = clean_skill(imp)
            equivalents.add(ic)
            for rev in _rev.get(ic, set()):
                equivalents.add(rev)
        mapping[jd_skill] = equivalents
    return mapping


# ── FIXED: normalised token matching, lowered thresholds ─────────────────────
def token_overlap_score(a, b):
    # Normalise hyphens/dots: "scikit-learn" == "scikit learn", "node.js" == "nodejs"
    def _ntok(s):
        return re.sub(r"[-./]", " ", s).lower().strip()
    ta = set(_ntok(a).split())
    tb = set(_ntok(b).split())
    if not ta or not tb:
        return 0.0
    return len(ta & tb) / len(ta | tb)


def skill_is_matched(jd_skill: str, jd_equivalents: set,
                     resume_expanded_set: set, resume_skill_list: list) -> tuple:
    rs = clean_skill(jd_skill)

    # Normalise helper — strip hyphens/dots for comparison
    def _n(s): return re.sub(r"[-./]", " ", s).strip()

    rs_n = _n(rs)
    resume_norm_set = {_n(x) for x in resume_expanded_set}

    # 1. Exact (original + normalised)
    if rs in resume_expanded_set:
        return True, "exact"
    if rs_n in resume_norm_set:
        return True, "exact_norm"

    # 2. Alias / equivalents (original + normalised)
    for eq in jd_equivalents:
        eq_n = _n(eq)
        if eq in resume_expanded_set or eq_n in resume_norm_set:
            return True, "alias"
        alias_of_eq = SKILL_ALIASES.get(eq)
        if alias_of_eq:
            if alias_of_eq in resume_expanded_set or _n(alias_of_eq) in resume_norm_set:
                return True, "alias"

    # 3. Token overlap — threshold lowered 0.35 → 0.25
    alias = SKILL_ALIASES.get(rs, rs)
    for res_sk in resume_skill_list:
        if token_overlap_score(rs, res_sk) >= 0.25:
            return True, "token"
        if token_overlap_score(alias, res_sk) >= 0.25:
            return True, "token"
        for eq in jd_equivalents:
            if token_overlap_score(eq, res_sk) >= 0.25:
                return True, "token"

    # 4. Substring — check both original and normalised forms
    for res_sk in resume_expanded_set:
        res_n = _n(res_sk)
        if rs in res_sk or res_sk in rs:
            return True, "substring"
        if len(rs_n) > 2 and (rs_n in res_n or res_n in rs_n):
            return True, "substring_norm"
        for eq in jd_equivalents:
            eq_n = _n(eq)
            if eq in res_sk or res_sk in eq:
                return True, "substring"
            if len(eq_n) > 2 and (eq_n in res_n or res_n in eq_n):
                return True, "substring_norm"
...
def match_skills(role_skills_raw, resume_skills_raw, emb_model):
    matched, missing, score, _, _ = match_skills_detailed(
        role_skills_raw, resume_skills_raw, emb_model
    )
    return matched, missing, score


def match_skills_detailed(role_skills_raw: str, resume_skills_raw: str, emb_model) -> tuple:
    ...
    jd_expansion = expand_jd_skills(role_list)
    ...
    for jd_sk in role_list:
        jd_equivalents = jd_expansion.get(jd_sk, {clean_skill(jd_sk)})
        is_match, reason = skill_is_matched(jd_sk, jd_equivalents, resume_exp, resume_list)
        if is_match:
            matched.append(jd_sk)
            match_reasons[jd_sk] = reason
            for eq in jd_equivalents:
                if eq in resume_exp:
                    alias_map[jd_sk] = eq
                    break
        else:
            needs_embed.append((jd_sk, jd_equivalents))

    if needs_embed and resume_list:
        embed_skills = [jd_sk for jd_sk, _ in needs_embed]
        try:
            role_vecs   = emb_model.embed_documents(embed_skills)
            resume_vecs = emb_model.embed_documents(resume_list)

            for i, (jd_sk, jd_equivalents) in enumerate(needs_embed):
                best_sim  = cosine_similarity([role_vecs[i]], resume_vecs)[0].max()
                best_idx  = cosine_similarity([role_vecs[i]], resume_vecs)[0].argmax()

                alias_forms = [eq for eq in jd_equivalents if eq != clean_skill(jd_sk)]
                if alias_forms:
                    try:
                        alias_vecs = emb_model.embed_documents(alias_forms)
                        for av in alias_vecs:
                            alias_sim = cosine_similarity([av], resume_vecs)[0].max()
                            if alias_sim > best_sim:
                                best_sim = alias_sim
                                best_idx = cosine_similarity([av], resume_vecs)[0].argmax()
                    except Exception:
                        pass

                # ── FIXED: threshold lowered 0.52 → 0.45 for better recall ──
                if best_sim >= 0.45:
                    matched.append(jd_sk)
                    match_reasons[jd_sk] = "semantic"
                    alias_map[jd_sk] = resume_list[best_idx]
        except Exception:
            pass

    missing = [r for r in role_list if r not in matched]
    score   = int(len(matched) / max(len(role_list), 1) * 100)
    return matched, missing, score, match_reasons, alias_map
...
def _extract_jd_skills_fast(jd_text: str) -> str:
    full = jd_text.lower()
    hits = []
    for kw in STRONG_TECH_KEYWORDS:
        ...
        if re.search(r"\b" + re.escape(kw) + r"\b", full):
            hits.append(kw)
    return ", ".join(hits) if hits else jd_text[:500]


def _compute_job_match(job: dict, resume_skills: str) -> int:
    if not resume_skills:
        return 0
    jd_text = (job.get("title", "") + " " + job.get("description", ""))
    jd_skills_str = _extract_jd_skills_fast(jd_text)
    if not jd_skills_str.strip():
        return 0
    try:
        matched, missing, score, _, _ = match_skills_detailed(jd_skills_str, resume_skills, embeddings)
        return min(score, 99)
    except Exception:
        resume_tokens = set(re.sub(r"[^a-z0-9\s]", " ", resume_skills.lower()).split())
        jd_tokens = set(re.sub(r"[^a-z0-9\s]", " ", jd_skills_str.lower()).split())
        stop = {"and","or","the","to","of","in","for","with","a","an","is","are"}
        resume_tokens -= stop; jd_tokens -= stop
        if not jd_tokens: return 0
        return min(int(len(resume_tokens & jd_tokens) / max(len(jd_tokens), 1) * 100), 99)
...
def extract_jd_skills_llm(jd_text: str) -> list:
    system = (
        "You are a technical skill extractor. "
        "Given a job description, output ONLY a JSON array of technical skills. "
        "Each item must be a short skill name (1-4 words max). "
        "NEVER include: salary, CTC, location, city names, degree names, college types, "
        "company names, job titles, years of experience numbers, "
        "full sentences, or non-technical words. "
        "Output raw JSON array only — no markdown, no explanation."
    )
    prompt = f"""Extract all technical skills from this job description.

Output format: ["skill1", "skill2", "skill3", ...]

Rules:
- Include: programming languages, frameworks, libraries, tools, platforms, methodologies, AI/ML concepts
- Exclude: CTC, salary, LPA, location, city, degree (BE/BTech/MCA/BCA/BSc), job title, company name,
  experience years, sentences, phrases longer than 4 words, non-technical adjectives
- Short form preferred: "RAG" not "retrieval augmented generation rag"
- Deduplicate
...
"""
    try:
        raw = _call_groq_direct(
            [{"role": "user", "content": prompt}],
            system_prompt=system,
            max_tokens=512, temperature=0.0, feature_tag="jd_skill_extractor"
        )
        raw = re.sub(r"```(?:json)?|```", "", raw).strip()
        skills_raw = json.loads(raw)
        if isinstance(skills_raw, list):
            _noise = re.compile(
                r"(?i)(lpa|ctc|salary|location|mumbai|bengaluru|bangalore|delhi|noida|"
                r"pune|chennai|gurgaon|hyderabad|pan india|b\.?e|b\.?tech|mca|bca|"
                r"bsc|bachelor|master|degree|fresher|intern|years?|months?|\d+\s*[-–]\s*\d+)"
            )
            cleaned = []
            seen = set()
            for s in skills_raw:
                s = str(s).strip().lower()
                if not s or len(s) < 2 or len(s.split()) > 5:
                    continue
                if _noise.search(s):
                    continue
                if s not in seen:
...
def extract_jd_skills(jd_text: str) -> str:
    skills = extract_jd_skills_llm(jd_text)
    return ", ".join(skills) if skills else jd_text[:500]
...
def llm_ats_analysis(resume_full_text, jd_text):
    ...
    # Step 1: Extract CLEAN skills from JD using Groq LLM
    jd_skill_list = extract_jd_skills_llm(jd_text)
    jd_skills_extracted = ", ".join(jd_skill_list)

    # Step 2: Multi-method skill matching
    matched, missing, real_score, match_reasons, alias_map = [], [], None, {}, {}
    jd_skill_count = len(jd_skill_list)

    try:
        matched, missing, real_score, match_reasons, alias_map = match_skills_detailed(
            jd_skills_extracted, resume_skills_raw, embeddings
        )
    except Exception:
        real_score = None

    def _is_clean_skill(s: str) -> bool:
        s = s.strip()
        if len(s.split()) > 5:
            return False
        _sentence_re = re.compile(
            r"(?i)(such as|using|where|e\.g|into|with|develop|build|deploy|"
            r"apply|manage|write|collaborate|experiment|integrate|lpa|ctc|"
            r"location|mumbai|bengaluru|noida|pune|chennai|gurgaon|"
            r"qualification|b\.e|b\.tech|mca|bca|bsc|\d+\s*/\s*\d+)"
        )
        if _sentence_re.search(s):
            return False
        return True

    matched_clean  = [sk for sk in matched if _is_clean_skill(sk)]
    missing_clean  = [sk for sk in missing if _is_clean_skill(sk)]

    matched_lines = []
    for sk in matched_clean[:20]:
        resume_form = alias_map.get(sk, "")
        if resume_form and clean_skill(resume_form) != clean_skill(sk):
            matched_lines.append(f"{sk}  (via alias: '{resume_form}')")
        else:
            method = match_reasons.get(sk, "")
            tag = {"exact": "", "alias": " (alias)", "exact_norm": " (norm)", "alias_norm": " (alias+norm)",
                   "token": " (partial)", "semantic": " (semantic)", "substring": " (substring)",
                   "substring_norm": " (substring+norm)"}.get(method, "")
            matched_lines.append(f"{sk}{tag}")

    alias_bridged = [
        f"JD '{sk}' ↔ resume '{alias_map[sk]}'"
        for sk in matched_clean
        if sk in alias_map and clean_skill(alias_map.get(sk, "")) != clean_skill(sk)
    ][:10]

    matched_str = "\n".join(f"  ✓ {l}" for l in matched_lines) if matched_lines else "  none detected"
    missing_str = "\n".join(f"  ✗ {l}" for l in missing_clean[:20]) if missing_clean else "  none detected"
    alias_note  = ("\nAlias-bridged matches:\n" + "\n".join(f"  • {a}" for a in alias_bridged)) if alias_bridged else ""

    score_note = (
        f"COMPUTED skill-overlap score: {real_score}% "
        f"({len(matched_clean)}/{jd_skill_count} JD skills matched — do not change this number)"
        if real_score is not None
        else "Skill overlap: could not be pre-computed — estimate carefully."
    )

    # ── FIXED: tell LLM that Overall ATS Score will be auto-computed ──────────
    score_instruction = (
        f"Estimate Skills Score, Experience Score, and Education Score carefully (each 0-100). "
        f"The Overall ATS Score will be AUTO-COMPUTED as their average — do NOT set it yourself. "
        f"Skill overlap is pre-computed at {real_score}% — use this as your Skills Score baseline (±5 only)."
        if real_score is not None
        else "Estimate Skills Score, Experience Score, and Education Score (0-100 each). Overall ATS Score will be computed as their average."
    )

    missing_plain = ", ".join(missing_clean[:20]) if missing_clean else "none"
...
- {score_note}
- Total JD skills identified (by LLM extraction): {jd_skill_count}
- JD Skills extracted: {jd_skills_extracted}

Matched skills (resume HAS these):
{matched_str}
{alias_note}

Missing skills (resume LACKS these):
{missing_str}
...
Overall ATS Score: <leave as 0 — will be replaced automatically>
...
Matched Skills:
<list only short skill names — NO sentences>
...
STRICT RULES:
- List ONLY short skill names (1-4 words) — NEVER full sentences
- Do NOT include CTC, salary, location, degree names, or job title phrases in any list
- Do NOT invent skills not mentioned in the JD
- Do NOT alter the pre-computed skill score by more than ±5
- Do NOT add URLs or course links

Resume:
{resume_full_text[:2500]}

Job Description:
{jd_text[:3000]}
"""
    result = safe_llm_invoke(prompt, feature_tag="ats_analysis")
    # ── FIXED: replace Overall ATS Score with average of sub-scores ──────────
    return _recompute_ats_overall_score(result)


# =========================================================
# FIXED: ATS overall score = average of sub-scores
# =========================================================
def _recompute_ats_overall_score(ats_text: str) -> str:
    """
    Parse Skills Score, Experience Score, Education Score from LLM output
    and replace Overall ATS Score with their mathematical average.
    Also appends a Score Audit block showing the calculation.
    """
    skills_m = re.search(r"Skills Score\s*:\s*(\d+)", ats_text, re.IGNORECASE)
    exp_m    = re.search(r"Experience Score\s*:\s*(\d+)", ats_text, re.IGNORECASE)
    edu_m    = re.search(r"Education Score\s*:\s*(\d+)", ats_text, re.IGNORECASE)

    if not (skills_m and exp_m and edu_m):
        return ats_text  # can't parse — return as-is

    s_score = int(skills_m.group(1))
    e_score = int(exp_m.group(1))
    d_score = int(edu_m.group(1))
    avg     = round((s_score + e_score + d_score) / 3)

    if avg >= 80:   decision = "Excellent Fit"
    elif avg >= 65: decision = "Good Fit"
    elif avg >= 45: decision = "Weak Fit"
    else:           decision = "Not a Fit"

    # Replace Overall ATS Score line
    ats_text = re.sub(
        r"Overall ATS Score\s*:\s*\d+",
        f"Overall ATS Score: {avg}",
        ats_text, flags=re.IGNORECASE
    )
    ...
    ats_text += (
        f"\n\n─── Score Audit ───\n"
        f"Skills Score:     {s_score}/100\n"
        f"Experience Score: {e_score}/100\n"
        f"Education Score:  {d_score}/100\n"
        f"Overall (avg):    {avg}/100  ←  ({s_score} + {e_score} + {d_score}) ÷ 3"
    )
    return ats_text
...
- Body 1: match 2-3 key skills/experiences FROM THE RESUME to JD requirements
- Body 2: highlight a specific achievement that ACTUALLY APPEARS in the resume
- Closing: call to action
- DO NOT use generic phrases like "I am writing to apply"
- DO NOT invent metrics unless they appear in the resume
...
def estimate_salary(skills: str, location: str, exp_years: int, exp_level: str,
    ...
    if real_salary_lines:
        grounding_block = "LIVE MARKET DATA:\n" + "\n".join(real_salary_lines[:5])
        data_note = "Base your range on the LIVE DATA above."
    else:
        grounding_block = "No live listings available."
        data_note = "No live data. Prefix every number with 'approx.' and add a disclaimer."

    system = (
        "You are a compensation analyst. Use only the data provided. "
...
Provide:
1. Expected CTC Range (in LPA)
2. Median Market Rate
3. Salary by City: Bangalore / Hyderabad / Mumbai / Pune / Remote
4. Negotiation Tips: 2-3 bullet points
5. Growth Projection: Expected CTC after 2-3 years

RULES: Use LPA throughout. Qualify all figures with "approx." if no live data.
"""
    return _call_groq_direct(
        [{"role": "user", "content": prompt}], system,
...
def generate_linkedin_bio(resume_text: str, resume_skills: str, exp_level: str,
    ...
    system = (
        ...
        "You NEVER invent companies, job titles, metrics, or achievements not in the resume."
    )
...
RULES: Only reference skills that appear above. Do NOT invent metrics.
"""
    return _call_groq_direct(
        [{"role": "user", "content": prompt}], system,
...
def compute_section5_scores(resume_full_text, jd_text, resume_skills_raw, emb_model):
    ...
    jd_coverage = None
    try:
        jd_skills_str = extract_jd_skills(jd_text)
        jd_skill_list = parse_skill_list(jd_skills_str)
...
def llm_transition_reason(resume_text, current_role, target_role, match_pct=None):
    if match_pct is not None:
        if match_pct < 35:
            honesty = f"The skill match is LOW ({match_pct}%). Be HONEST — clearly acknowledge the significant skill gap."
        elif match_pct < 60:
            honesty = f"The skill match is MODERATE ({match_pct}%). Be BALANCED — mention both strengths and gaps."
        else:
            honesty = f"The skill match is STRONG ({match_pct}%). Highlight genuine alignment."
    else:
    ...
    prompt = (
        ...
        f"RULES: Only reference skills that ACTUALLY APPEAR in the resume. "
        f"Do NOT add URLs or certification links."
    )
...
def llm_learning_roadmap(target_role, missing_skills):
    ...
    prompt = (
        ...
        f"RULES:\n"
        f"- Do NOT invent URLs or hyperlinks\n"
        f"- Say 'search for X on Coursera/Udemy/YouTube' — never give a fake link\n"
        f"- Include 2-3 realistic project ideas relevant to {target_role}\n"
        f"- Timeline: use ranges like 'typically 4-8 weeks'"
    )
...
def _process_single_url(role_name: str, url: str, label: str = "") -> dict:
    ...
    try:
        ...
        prompt = (
            ...
            f"Based on the URL and platform below, write exactly 3 sentences: "
            f"(1) what type of resource this likely is, "
            f"(2) what topic or skill it probably covers, "
            f"(3) why it is useful for {role_name}. "
            f"Do NOT invent course names or instructors. "
            f"URL: {url} | Platform: {platform} | Title hint: {title}"
        )
        summary = _call_groq_direct(
            [{"role": "user", "content": prompt}], system,
            max_tokens=180, temperature=0.3, feature_tag="url_summarizer",
        )
    except Exception:
        summary = f"This {platform} resource covers topics relevant to {role_name}."

    if not summary or summary.startswith("⚠️"):
        summary = f"A {platform} resource covering key concepts for {role_name}. Click to learn more."

    return {
        "url": url, "label": label, "platform": platform,
        "resource_type": resource_type, "title": title, "summary": summary,
    }
...
def _process_url_batch(role_name: str, batch: list) -> list:
    ...
    for item in batch:
        ...
        try:
            ...
            card = {
                "url": url, "label": label, "platform": "Generic",
                "resource_type": "page",
                "title": label or _title_from_url(url),
                "summary": "Could not generate summary. Visit the link directly.",
            }
        cards.append(card)
    return cards
...
3. For each role, list the top 5 skills to focus on first.
4. Give a realistic learning timeline.
...
- NEVER mention a specific company as "who is hiring" — you don't have live data
- NEVER give exact salary figures — use ranges and say "approximately"
- NEVER invent a course URL or platform link
- NEVER recommend a role NOT listed in AVAILABLE ROLES above
- If uncertain, say "I'd recommend verifying this directly on the platform"
"""
...
def _inject_resume_context(messages: list) -> list:
    ...
    enriched = [
        ...
        {"role": "assistant", "content": "Got it — I've reviewed your background. How can I help you today?"},
    ]
...
def render_home():
    ...
    if ls_active:
        ...
        <div style="background:#080f18; border:1px solid #1a2535; border-radius:12px;
            ...
            <span style="font-size:0.78rem; color:#4B5563;">⚫ LangSmith Tracing: Off — Add LANGCHAIN keys to .env to enable</span>
        </div>""", unsafe_allow_html=True)
    ...
    if ntfy_topic:
        ...
        <div style="background:#080f18; border:1px solid #1a2535; border-radius:12px;
            ...
            <span style="font-size:0.78rem; color:#4B5563;">🔕 Push Alerts: Off — Add NTFY_TOPIC to .env to enable</span>
        </div>""", unsafe_allow_html=True)
...
def _render_resource_card(card: dict) -> None:
    ...
    <div class="resource-card">
        ...
        <div style="margin-top:10px;">
            <a href="{url}" target="_blank" style="font-size:0.75rem; color:#22D3EE; font-weight:600;">
               🔗 Open Resource →
            </a>
        </div>
    </div>
...
def render_skill_match_app():
    ...
    if st.button("Run Match Analysis", type="primary"):
        ...
        if rdata:
            matched, missing, score, match_reasons, alias_map = match_skills_detailed(
                rdata["skills"], st.session_state.resume_skills, embeddings
            )
            ...
            with col1:
                ...
                for sk in sorted(set(matched)):
                    resume_form = alias_map.get(sk, "")
                    if resume_form and clean_skill(resume_form) != clean_skill(sk):
                        st.markdown(f"- {sk} *(resume: '{resume_form}')*")
                    else:
                        st.markdown(f"- {sk}")
            with col2:
                st.markdown("**📚 Skills to Acquire**")
                if missing:
                    for sk in sorted(set(missing)):
                        st.markdown(f"- {sk}")
                else:
                    st.success("You have all required skills!")

            alias_bridges = [
                f"**{sk}** ← resume used *'{alias_map[sk]}'*"
                for sk in matched
                if sk in alias_map and clean_skill(alias_map.get(sk,"")) != clean_skill(sk)
            ]
            if alias_bridges:
                with st.expander(f"ℹ️ {len(alias_bridges)} alias-matched skills"):
                    st.caption("These JD skills were matched via abbreviation/alias.")
                    for ab in alias_bridges:
                        st.markdown(f"- {ab}")
...
def render_ats_app():
    ...
    st.markdown(
        '<div class="screen-subheader">Groq LLM extracts skills from JD → multi-method matching → ATS score = avg(Skills + Experience + Education)</div>',
        unsafe_allow_html=True
    )
    ...
    jd_tab_upload, jd_tab_paste = st.tabs(["📄 Upload JD PDF", "📋 Paste JD Text"])
    with jd_tab_upload:
        ...
        if jd_file:
            ...
            st.session_state.ats_jd_skills = []
            st.session_state.ats_result    = None
            st.success(f"JD uploaded — {len(jd_text)} characters extracted")

    with jd_tab_paste:
        pasted = st.text_area(
            "Paste JD text here",
            value=st.session_state.jd_text[:3000] if st.session_state.jd_text else "",
            height=180, key="jd_paste_area",
            placeholder="Paste the full job description text…"
        )
        if st.button("Use This JD", key="use_pasted_jd"):
            if pasted.strip():
                st.session_state.jd_text       = pasted.strip()
                st.session_state.ats_jd_skills = []
                st.session_state.ats_result    = None
                st.success("JD text saved.")

    if st.session_state.resume_text and st.session_state.jd_text:

        jd_skills_preview = st.session_state.get("ats_jd_skills", [])

        col_ext, col_clear = st.columns([3, 1])
        with col_ext:
            if st.button("🔍 Extract JD Skills with Groq", key="extract_jd_skills_btn"):
                with st.spinner("Groq LLM extracting skills from JD…"):
                    extracted = extract_jd_skills_llm(st.session_state.jd_text)
                st.session_state.ats_jd_skills = extracted
                jd_skills_preview = extracted
        with col_clear:
            if st.button("Clear", key="clear_jd_skills"):
                st.session_state.ats_jd_skills = []
                st.session_state.ats_result    = None
                st.rerun()

        if jd_skills_preview:
            st.markdown("**✅ JD Skills Extracted by Groq LLM**")
            st.markdown(
                '<div style="background:#030d08; border:1px solid #0a2e18; border-left:3px solid #22D3EE; '
                'border-radius:8px; padding:12px 16px; font-size:0.82rem; color:#6ee7b7; line-height:1.8;">'
                + " &nbsp;·&nbsp; ".join(
                    f'<span style="background:#052e16; padding:2px 8px; border-radius:6px; '
                    f'border:1px solid #16a34a;">{sk}</span>'
                    for sk in jd_skills_preview
                )
                + f'<br><span style="color:#4B5563; font-size:0.72rem;">{len(jd_skills_preview)} skills identified</span>'
                + '</div>',
                unsafe_allow_html=True
            )
            st.markdown("<br>", unsafe_allow_html=True)

        if st.button("🚀 Run Full ATS Analysis", type="primary", key="run_ats_btn"):
            with st.spinner("Step 1/3 — Extracting JD skills with Groq…"):
                if not st.session_state.get("ats_jd_skills"):
                    st.session_state.ats_jd_skills = extract_jd_skills_llm(st.session_state.jd_text)
            with st.spinner("Step 2/3 — Running multi-method skill matching…"):
                pass
            with st.spinner("Step 3/3 — Generating ATS report with Groq…"):
                ats_text = llm_ats_analysis(
        ...
        if st.session_state.ats_result:
            st.markdown("---")
            tab_report, tab_metrics, tab_debug = st.tabs(
                ["📋 ATS Report", "📊 Semantic Alignment", "🔬 Debug — Extracted Skills"]
            )
            with tab_report:
                st.markdown(
                    '<div style="background:#080f18; border:1px solid #1a2535; border-left:4px solid #22D3EE; '
                    'border-radius:12px; padding:22px; font-size:0.88rem; line-height:1.8; '
                ...
                st.download_button(
                    "📥 Download ATS Report", data=st.session_state.ats_result,
            ...
            with tab_metrics:
                ...
                st.caption("Semantic match = cosine similarity between full resume and JD text vectors.")

            with tab_debug:
                st.markdown("#### 🔬 What Groq LLM extracted from the JD")
                jd_skills = st.session_state.get("ats_jd_skills", [])
                if jd_skills:
                    for i, sk in enumerate(jd_skills, 1):
                        st.markdown(f"`{i}.` **{sk}**")
                else:
                    st.info("Run 'Extract JD Skills' or 'Run Full ATS Analysis' first.")
                st.markdown("#### 📄 Raw JD Text (first 1500 chars)")
                st.text(st.session_state.jd_text[:1500])

    elif not st.session_state.jd_text:
        st.info("Upload a JD PDF or paste JD text above to get started.")
...
def render_market_app():
    ...
    st.markdown('<div class="screen-subheader">Live job counts per role from Adzuna India</div>', unsafe_allow_html=True)
    ...
    selected_roles = st.multiselect("Roles to analyse", sorted(role_names),
                                     default=safe_default, key="market_roles_select")
    if selected_roles:
    ...
    if insights:
        ...
        if sorted_insights:
            ...
            bars = ax.barh(roles_list[::-1], counts_list[::-1], color=colors[::-1], height=0.55, edgecolor="#1f1f1f")
            for bar, count in zip(bars, counts_list[::-1]):
                ax.text(bar.get_width() + max(counts_list) * 0.01,
                        bar.get_y() + bar.get_height() / 2,
                        f"{count:,}", va="center", ha="left", color="#9CA3AF", fontsize=8)
        ...
        st.success(f"🔥 **Hottest role right now:** {top_role} with **{top_count:,}** open positions!")
        st.caption("Powered by Adzuna Jobs API · Counts reflect listings at time of query")
    else:
        st.info("Click 'Fetch Live Job Counts' to load real-time data.")


# =========================================================
# SCREEN: SALARY TRENDS
# =========================================================
def render_salary_chart_app():
    back_button()
    st.markdown('<div class="screen-header">Salary Trends</div>', unsafe_allow_html=True)
    st.markdown('<div class="screen-subheader">Average salary comparison across your top role recommendations</div>', unsafe_allow_html=True)

    if not st.session_state.resume_skills:
        st.warning("No resume loaded. Please go to Resume Analyzer first.")

    selected_roles_sal = st.multiselect("Select roles to compare", sorted(role_names),
                                         key="salary_roles_select")
    ...
    if salary_data:
        ...
        bars = ax.bar(range(len(roles_list)), lpa_list, color=bar_colors, width=0.55, edgecolor="#1f1f1f")
        ...
        ax.set_xticklabels([r[:18] for r in roles_list], rotation=20, ha="right", color="#9CA3AF", fontsize=8)
        ax.set_ylabel("LPA", color="#9CA3AF", fontsize=9)
        ...
        for role, lpa in sorted_sal:
            with st.expander(f"💼 {role} — ₹{lpa} LPA average"):
                st.markdown(f"**Range:** ₹{round(lpa*0.75,1)} – ₹{round(lpa*1.35,1)} LPA")
                st.markdown(f"**Fresher entry:** ₹{round(lpa*0.55,1)} – ₹{round(lpa*0.75,1)} LPA")
                st.markdown(f"**3-5 years:** ₹{round(lpa*0.90,1)} – ₹{round(lpa*1.25,1)} LPA")
                st.markdown(f"**5+ years / senior:** ₹{round(lpa*1.25,1)} – ₹{round(lpa*1.8,1)} LPA")
        st.caption("⚠️ Estimates based on Adzuna listings + Indian market benchmarks. Actual offers vary.")
    else:
        st.info("Click 'Generate Salary Chart' to compare salaries.")


# =========================================================
# SCREEN: AI COVER LETTER
# =========================================================
def render_coverletter_app():
    ...
    if not st.session_state.jd_text:
        st.info("ℹ️ No JD loaded. Upload via ATS Analysis or paste below.")
    ...
    jd_input = st.text_area(
        ...
        height=140, key="cl_jd_input", placeholder="Paste the job description here…"
    )
    if jd_input.strip():
    ...
    col1, col2 = st.columns(2)
    with col1:
        cl_name = st.text_input("Your Name", value=st.session_state.get("candidate_name", ""), key="cl_name")
    with col2:
    ...
    if st.button("✍️ Generate Cover Letter", type="primary"):
        if not st.session_state.resume_text:
            ...
            with st.spinner("Generating cover letter with Groq AI…"):
                cover_letter = generate_cover_letter(
                    st.session_state.resume_text, st.session_state.jd_text,
                    cl_name or "Applicant", exp_label, cl_location
                )
    ...
    if st.session_state.get("cover_letter"):
        ...
        <div style="background:#080f18; border:1px solid #1a2535; border-left:4px solid #22D3EE;
                    ...
                    color:#E6E6E6; white-space:pre-wrap;">{st.session_state.cover_letter}</div>
        """, unsafe_allow_html=True)
        col_copy, col_dl = st.columns(2)
        with col_dl:
            st.download_button("📥 Download Cover Letter (.txt)", data=st.session_state.cover_letter,
                               file_name="cover_letter.txt", mime="text/plain")
        with col_copy:
            if st.button("🔄 Regenerate", key="regen_cl"):
                st.session_state.cover_letter = ""
                st.rerun()
        st.caption("💡 Tip: Customise the letter further for each application.")
...
def render_salary_est_app():
    ...
    st.markdown('<div class="screen-subheader">AI-powered salary range based on your skills, location & experience</div>', unsafe_allow_html=True)
    ...
    if st.button("💰 Estimate My Salary", type="primary"):
        if not sal_skills.strip():
            ...
            with st.spinner("Analysing market rates with Groq AI…"):
                estimate = estimate_salary(sal_skills, sal_location, int(sal_exp_years),
...
{st.session_state.salary_estimate}
        ...
        """, unsafe_allow_html=True)
        st.download_button("📥 Download Salary Report", data=st.session_state.salary_estimate,
                           file_name="salary_estimate.txt", mime="text/plain")
        st.caption("⚠️ AI-generated estimates. Actual offers depend on company and negotiation.")
...
def render_linkedin_app():
    ...
    if st.session_state.get("linkedin_bio"):
        ...
        <div style="background:#080f18; border:1px solid #1a2535; border-left:4px solid #22D3EE;
                    ...
                    color:#E6E6E6; white-space:pre-wrap;">{st.session_state.linkedin_bio}</div>
        """, unsafe_allow_html=True)
        col_dl, col_regen = st.columns(2)
        with col_dl:
            st.download_button("📥 Download Bio (.txt)", data=st.session_state.linkedin_bio,
                               file_name="linkedin_about.txt", mime="text/plain")
        with col_regen:
            if st.button("🔄 Regenerate Bio", key="regen_li"):
                st.session_state.linkedin_bio = ""
                st.rerun()
        st.caption("💡 Copy into LinkedIn → Edit Profile → About section.")
...
5. Make sure key skills appear in the first 2 lines
            """)
...
def render_assistant_app():
    ...
    st.markdown(f'<div class="screen-subheader">AI career advisor · Groq Llama-3.3-70B · {len(role_names)} roles</div>', unsafe_allow_html=True)

    has_resume = bool(st.session_state.get("resume_text"))
    res_status = "✓ Resume loaded — personalised mode" if has_resume else "No resume · upload via Resume Analyzer"
...
# =========================================================
_EXP_LEVELS = {
    "fresher": ("🌱 Fresher / Recent Graduate", "0–1 year · internships, entry-level & fresher roles"),
    "junior":  ("🔰 Junior",                    "1–3 years · junior & associate-level roles"),
    "mid":     ("⚡ Mid-Level",                  "3–6 years · individual contributor & specialist"),
    "senior":  ("🚀 Senior / Lead",              "6+ years · senior, lead & principal roles"),
}
...
def render_jobs_app():
    ...
    st.markdown('<div class="screen-subheader">Real jobs from India · Skill match score · 12-hr filter · Applied history · Adzuna</div>', unsafe_allow_html=True)
    ...
    filter_12h = st.toggle(
        "⚡ Last 12 hours only",
        value=st.session_state.get("jobs_filter_12h", False),
        key="jobs_filter_12h",
        help="Only show jobs posted in the past 12 hours. May return fewer results — turn OFF to see more."
    )
    hours_filter = 12 if filter_12h else None
    ...
    if st.session_state.get("job_results"):
        ...
        filter_note = " · ⏱ Last 12 hrs" if filter_12h else ""
        st.markdown(f'<p class="section-hint">{len(valid_jobs)} <strong>{exp_label}</strong> jobs · '
                    f'<strong>{role_shown}</strong> · {loc_shown.title()} · Page {page_shown}{match_note}{filter_note}</p>',
                    unsafe_allow_html=True)
    ...
    if applied_jobs:
        ...
        for idx, app in enumerate(reversed(applied_jobs), 1):
            ...
            <div class="job-card" style="padding:12px 16px;">
                ...
                <div class="job-meta" style="margin-top:4px;">🏢 {app.get('company','—')} &nbsp;·&nbsp; 🔍 {app.get('role_searched','—')}</div>
...
def render_feedback_app():
    ...
    with left_col:
        ...
        with st.form("feedback_form", clear_on_submit=True):
            ...
            fb_comment = st.text_area("Your Comment",
                                       placeholder="Tell us what worked well, what could be better…",
                                       height=100)
            fb_tags_chosen = st.multiselect("Quick Tags (optional)", FEEDBACK_TAGS, max_selections=3)
    ...
    with right_col:
        ...
        with fc1:
            filter_feature = st.selectbox("Filter by feature", ["All"] + FEEDBACK_FEATURES,
                                           key="fb_filter", label_visibility="collapsed")
        with fc2:
            sort_by = st.selectbox("Sort", ["newest", "oldest", "highest", "lowest", "most liked"],
        ...
        if not reviews:
            ...
            <div style="text-align:center; padding:60px 20px; color:#4B5563;">
                ...
                <div style="font-size:0.9rem; color:#6B7280;">No reviews yet. Be the first to share!</div>
            </div>""", unsafe_allow_html=True)
        else:
...
1. **Resume Analyzer** — PDF upload, skill extraction, Resume Score Card (0–100, grade A/B/C/D)
2. **Career Roles Explorer** — Browse {len(role_names)} roles with roadmaps and resource summaries
3. **Skill Match & Gap** — 4-method cascade matching (exact / alias+norm / token overlap / semantic)
4. **Top Role Suggestions** — AI-ranked recommendations from your resume
5. **Career Transitions** — LLM-powered gap analysis with learning roadmap
6. **ATS Analysis** — Overall Score = avg(Skills + Experience + Education) ✓ FIXED
7. **Readiness Dashboard** — 6-dimension career readiness radar chart
8. **Career Assistant** — Groq Llama-3.3-70B chatbot with resume context
9. **Job Listings** — Adzuna live jobs with skill match % per listing + 12-hr filter
...
12. **AI Cover Letter** — Groq-powered cover letter from resume + JD
13. **Salary Estimator** — Salary range estimate by skills + location + experience
14. **LinkedIn Bio Writer** — AI rewrites resume into LinkedIn About section
15. **Feedback & Community** — Star ratings, comments, tag-based reviews, like system

**Models & APIs**
- Career Assistant + Cover Letter + Salary + LinkedIn: Groq Llama-3.3-70B
...
**Skill Matching Fixes (v2)**
- Overall ATS Score = avg(Skills Score + Experience Score + Education Score)
- Bidirectional alias expansion: resume 'sklearn' now matches JD 'scikit-learn' and vice versa
- Normalised comparison: 'scikit-learn' == 'scikit learn', 'node.js' == 'nodejs'
- Token overlap threshold lowered 0.35 → 0.25 (catches 'deep learning' vs 'dl')
- Embedding cosine threshold lowered 0.52 → 0.45 (better semantic recall)
- 60+ new bidirectional aliases: fine-tuning, huggingface, rag, llms, power bi, mlops, lora, etc.
- Expanded IMPLIES: huggingface→fine-tuning, rag→langchain, lora→pytorch, mlops→docker
